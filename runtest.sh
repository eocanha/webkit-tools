...
export GST_DEBUG="*:WARN,webkit*:TRACE"
...
OPT_MSE="--enable-mediasource=true --enable-write-console-messages-to-stdout=true"
...
run-minibrowser --gtk ${OPT_DEBUG} ${OPT_MSE} "${OPT_USER_AGENT}" ${FULLSCREEN_OPTION} "${URL}" 2>&1 | tee /tmp/log.txt
postprocess
